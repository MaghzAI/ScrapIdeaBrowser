...
class StreamlitScraperApp:
    ...
    def zip_folder(self, folder_path):
        """ضغط المجلد المحدد إلى ملف ZIP داخل ScrapContent"""
        zip_name = os.path.basename(folder_path) + "_zip.zip"
    ...
    def show_archives_table(self):
        ...
        if self.archives:
            ...
            def make_download_link(zip_path):
                # رابط التحميل يكون دائماً /ScrapContent/folder_name_zip.zip
                rel_zip = os.path.relpath(zip_path, os.getcwd())
                url = f"/{rel_zip}"
                return f'<a href="{url}" download>تحميل</a>'
